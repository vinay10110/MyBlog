...
export default function PostPage() {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(`${process.env.REACT_APP_SERVER_ADDRESS}/api/post/${id}`)
      .then(response => {
  ...
  async function deletePost() {
    ...
    const response = await fetch(`${process.env.REACT_APP_SERVER_ADDRESS}/api/post`, {
      method: 'DELETE',
      headers: {
  ...
  return (
    <div>
      ...
      {userInfo && userInfo.id === postInfo.author._id && (
          ...
          <a href='/' onClick={deletePost} className="edit-btn"><svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="50" height="500" viewBox="0 0 100 100">
